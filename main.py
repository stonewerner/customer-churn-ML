...
def make_predictions(input_df, input_dict):
    ...
    probabilities = {
        ...
        # 'VotingCLF': voting_classifier_model.predict_proba(input_df)[0][1],
    }
...
def generate_email(probability, input_dict, explanation, surname):
    prompt = f"""You are a manager at a bank. You are responsible for ensuring customers stay with the bank and are incentivized with various offers.

    You noticed a customer named {surname} has a {round(probability*100, 1)}% probability of churning, based on the information provided below.

    Here is the customer informtaion:
    {input_dict}

    Here is the explanation of the customer's churning probability:
    {explanation}

    Generate an email to this customer based on their information, asking them to stay if they are at risk of churning, or offering them incentives so that they become more loyal to the bank.
